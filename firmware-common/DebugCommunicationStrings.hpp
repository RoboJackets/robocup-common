...
const std::map<std::string, DebugResponse> STRING_TO_DEBUGRESPONSE = []() {
    ...
    debugThrowIf("Enums missing from RESPONSE_TO_STRING",
                 DEBUGRESPONSE_TO_STRING.size() != RESPONSE_INFO.size());
    return m;
}();
...
const std::map<std::string, ConfigCommunication> STRING_TO_CONFIG = []() {
    ...
    debugThrowIf("Enums missing from STRING_TO_CONFIG",
                 STRING_TO_CONFIG.size() != CONFIG_INFO.size());
    return m;
}();
}